//ff-c++-LIBRARY-dep: cxx11 [petsccomplex hpddm] [mkl|blas] mpi pthread htool bemtool boost
//ff-c++-cpp-dep:
...
public:
    HMatrixImpl(double epsilon=1e-6 ,double eta=10, char symmetry='N', char UPLO='N',const int& reqrank=-1, MPI_Comm comm=MPI_COMM_WORLD) : H(3,epsilon,eta,symmetry,UPLO,reqrank,comm){}
    ...
    void set_minclustersize(unsigned int minclustersize0) {H.set_minclustersize(minclustersize0);}   
...
AnyType SetCompressMat(Stack stack,Expression emat,Expression einter,int init)
{
  ...
  if ( compressor == "" || compressor == "partialACA")
       *Hmat = new HMatrixImpl<K,partialACA>(epsilon,eta);
   else if (compressor == "fullACA")
       *Hmat = new HMatrixImpl<K,fullACA>(epsilon,eta);
   else if (compressor == "SVD")
       *Hmat = new HMatrixImpl<K,SVD>(epsilon,eta);
   else {
...
void builHmat(HMatrixVirt<R>** Hmat, IMatrix<R>* generatorP,string compressor,vector<double> &p1,vector<double> &p2,MPI_Comm comm)  {
    if (compressor=="" || compressor == "partialACA")
        *Hmat = new HMatrixImpl<R,partialACA>(1e-3,10,'N','N',-1,comm);
    else if (compressor == "fullACA")
        *Hmat = new HMatrixImpl<R,fullACA>(1e-3,10,'N','N',-1,comm);
    else if (compressor == "SVD")
        *Hmat = new HMatrixImpl<R,SVD>(1e-3,10,'N','N',-1,comm);
    else {
        cerr << "Error: unknown htool compressor \""+compressor+"\"" << endl;
        ffassert(0);
    }
    ...
    *Hmat =0;
    ...
    if (VFBEM==1) {
        ...
        if ( ds.compressor == "" || ds.compressor == "partialACA")
            *Hmat = new HMatrixImpl<R,partialACA>(ds.epsilon,ds.eta,ds.sym?'S':'N',ds.sym?'U':'N',-1,comm);
        else if (ds.compressor == "fullACA")
            *Hmat = new HMatrixImpl<R,fullACA>(ds.epsilon,ds.eta,ds.sym?'S':'N',ds.sym?'U':'N',-1,comm);
        else if (ds.compressor == "SVD")
            *Hmat = new HMatrixImpl<R,SVD>(ds.epsilon,ds.eta,ds.sym?'S':'N',ds.sym?'U':'N',-1,comm);
        else {
            cerr << "Error: unknown htool compressor \""+ds.compressor+"\"" << endl;
            ffassert(0);
        }
        (*Hmat)->set_minclustersize(ds.minclustersize);
